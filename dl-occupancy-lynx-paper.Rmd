...
```{r figure2, fig.cap="**Figure 2**: Detections (black) and non-detections (light grey) for each of the 3 species lynx, chamois and roe deer. Sites are on the Y axis, while sampling occasions are on the X axis.", fig.align="center"}
## Occupancy analyses
...
df$model <- rep(c('manual tagging', 'automatic tagging'), c(nrow(conditional1), nrow(conditional2)))
...
df$model <- rep(c('manual tagging', 'automatic tagging'), c(nrow(marginal1), nrow(marginal2)))
df$param_f <- factor(df$param, 
