...
plot_row
#ggsave(filename = "img/fig1.png", plot = plot_row, scale = 2)
```

```{r studysite2, fig.cap = "**Figure 1**: Study area, grid and camera trap locations."}
...
```{r lasso, include = FALSE, eval = FALSE}
set.seed(2022)
kk <- 10
# y <- list(lynx = lynx, deer = deer, chamois = chamois, fox = fox, cat = cat)
...
```{r dircreate, echo=TRUE, eval=FALSE}
...
```{r onecycle, echo = TRUE, cache = FALSE}
one_cycle <- learn %>%
